...
if [[ "$DEFAULT_HERMES_EXE" != "$HOME"/* ]]; then
  echo "WARNING: hermes executable is not under $HOME: $DEFAULT_HERMES_EXE" >&2
  echo "This looks like a system-wide install. We strongly recommend a per-user install (e.g. ~/.local/bin/hermes)." >&2
  if [[ "$YES" -eq 0 ]]; then
