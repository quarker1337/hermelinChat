...
const Toggle = ({ on, onToggle, label, sublabel }) => (
  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0" }}>
    <div>
      <div style={{ fontSize: 12, color: SLATE.textBright, fontWeight: 500 }}>{label}</div>
      {sublabel && <div style={{ fontSize: 10, color: SLATE.muted, marginTop: 2 }}>{sublabel}</div>}
    </div>
    <div
      onClick={onToggle}
      style={{
        width: 36, height: 20, borderRadius: 10, cursor: "pointer",
        background: on ? AMBER[500] : SLATE.border,
        transition: "background 0.2s ease", position: "relative", flexShrink: 0,
      }}
    >
      <div style={{
        width: 16, height: 16, borderRadius: "50%", background: on ? "#fff" : SLATE.muted,
...
const SelectField = ({ label, value, options, onChange }) => (
  <div style={{ padding: "8px 0" }}>
    <div style={{ fontSize: 12, color: SLATE.textBright, fontWeight: 500, marginBottom: 6 }}>{label}</div>
    <select
      value={value}
      onChange={(e) => onChange(e.target.value)}
      style={{
        width: "100%", padding: "7px 10px", borderRadius: 6,
        background: SLATE.elevated, border: `1px solid ${SLATE.border}`,
        color: SLATE.textBright, fontSize: 12, fontFamily: "'JetBrains Mono',monospace",
        outline: "none", cursor: "pointer",
        WebkitAppearance: "none", MozAppearance: "none", appearance: "none",
        backgroundImage: `url("data:image/svg+xml,%3Csvg width='10' height='6' viewBox='0 0 10 6' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1L5 5L9 1' stroke='%2355556a' stroke-width='1.5' stroke-linecap='round'/%3E%3C/svg%3E")`,
        backgroundRepeat: "no-repeat", backgroundPosition: "right 10px center",
      }}
    >
      {options.map((o) => (
        <option key={o.value} value={o.value} style={{ background: SLATE.surface, color: SLATE.textBright }}>
          {o.label}
        </option>
      ))}
    </select>
...
// ─── TEXT INPUT FIELD ─────────────────────────────────────────────
const TextField = ({ label, value, onChange, placeholder, type = "text", mono }) => (
  <div style={{ padding: "8px 0" }}>
    <div style={{ fontSize: 12, color: SLATE.textBright, fontWeight: 500, marginBottom: 6 }}>{label}</div>
    <input
      type={type}
      ...
      style={{
        width: "100%", padding: "7px 10px", borderRadius: 6,
        background: SLATE.elevated, border: `1px solid ${SLATE.border}`,
        color: SLATE.textBright, fontSize: 12,
...
const SettingsSection = ({ title, icon, children, defaultOpen = false }) => {
  ...
  return (
    <div style={{ borderBottom: `1px solid ${SLATE.border}` }}>
      <div
        onClick={() => setOpen(!open)}
        style={{
          display: "flex", alignItems: "center", gap: 8, padding: "12px 16px",
          cursor: "pointer", fontSize: 12, fontWeight: 600, color: SLATE.textBright,
          fontFamily: "'JetBrains Mono',monospace",
        }}
        onMouseEnter={(e) => e.currentTarget.style.background = SLATE.elevated}
...
const SettingsPanel = ({ open, onClose }) => {
  const [model, setModel] = useState("z-ai/glm-5");
  const [systemPrompt, setSystemPrompt] = useState("You are Hermelin, an autonomous agent...");
  ...
  const [particlesBg, setParticlesBg] = useState(true);
  const [skillAutolearn, setSkillAutolearn] = useState(true);
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: SLATE.surface, borderLeft: `1px solid ${SLATE.border}`,
        zIndex: 101, display: "flex", flexDirection: "column",
        animation: "settingsSlide 0.25s cubic-bezier(0.16,1,0.3,1) both",
        boxShadow: "-8px 0 32px rgba(0,0,0,0.4)",
      }}>
        {/* Header */}
        <div style={{
          padding: "16px 16px 14px", borderBottom: `1px solid ${SLATE.border}`,
          ...
          <div
            onClick={onClose}
            style={{
              cursor: "pointer", color: SLATE.muted, padding: 4, borderRadius: 4,
              display: "flex", alignItems: "center", justifyContent: "center",
            }}
            onMouseEnter={(e) => { e.currentTarget.style.color = SLATE.textBright; e.currentTarget.style.background = SLATE.elevated; }}
            onMouseLeave={(e) => { e.currentTarget.style.color = SLATE.muted; e.currentTarget.style.background = "transparent"; }}
          >
            <CloseIcon />
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflow: "auto" }}>
          ...
          <SettingsSection title="Model" icon="🧠" defaultOpen={true}>
            <SelectField
              ...
              options={[
                ...
                { value: "nous/hermes-3-70b", label: "Nous / Hermes 3 70B" },
                { value: "anthropic/claude-sonnet", label: "Anthropic / Claude Sonnet" },
                { value: "openai/gpt-4o", label: "OpenAI / GPT-4o" },
                { value: "google/gemini-2.5-pro", label: "Google / Gemini 2.5 Pro" },
                { value: "local/qwen-72b", label: "Local / Qwen 72B (vLLM)" },
              ]}
            />
            <TextField
              label="Temperature"
            ...
            <TextField
              ...
              mono
            />
            <Toggle label="Stream responses" on={streamResponse} onToggle={() => setStreamResponse(!streamResponse)} />
          </SettingsSection>

          <SettingsSection title="Agent" icon="⚡">
            <Toggle
              label="Auto-title sessions"
              sublabel="Generate title from first message"
              on={autoTitle}
              onToggle={() => setAutoTitle(!autoTitle)}
            />
            <Toggle
              label="Skill auto-learn"
              sublabel="Create skills after solving novel problems"
              on={skillAutolearn}
              onToggle={() => setSkillAutolearn(!skillAutolearn)}
            />
            <Toggle
              label="Confirm dangerous actions"
              sublabel="Prompt before rm, kubectl delete, etc."
              on={confirmDangerous}
              onToggle={() => setConfirmDangerous(!confirmDangerous)}
            />
            <Toggle
              label="Sandbox mode"
              sublabel="Restrict file/network access"
              on={sandboxMode}
              onToggle={() => setSandboxMode(!sandboxMode)}
            />
            <Toggle
              label="PTY mode"
              sublabel="Full terminal emulation"
              on={ptyMode}
              onToggle={() => setPtyMode(!ptyMode)}
            />
          </SettingsSection>
          ...
          <SettingsSection title="Appearance" icon="🎨">
            <Toggle
              ...
              on={particlesBg}
              onToggle={() => setParticlesBg(!particlesBg)}
            />
            <Toggle
              label="CRT scanlines"
              on={scanlines}
            ...
            <Toggle
              label="Sound effects"
              sublabel="Key clicks, notification chimes"
              on={soundEffects}
            ...
            <SelectField
              ...
              onChange={() => {}}
              options={[
                { value: "amber-dark", label: "Amber Dark (default)" },
                { value: "green-matrix", label: "Green Matrix" },
                { value: "blue-ice", label: "Blue Ice" },
                { value: "red-alert", label: "Red Alert" },
                { value: "mono-white", label: "Monochrome" },
              ]}
            />
          </SettingsSection>

          <SettingsSection title="History & Data" icon="💾">
            <Toggle
            ...
            />
            <div style={{ padding: "8px 0" }}>
              <div style={{ fontSize: 12, color: SLATE.textBright, fontWeight: 500, marginBottom: 6 }}>Session storage</div>
              <div style={{ fontSize: 11, color: SLATE.muted, fontFamily: "'JetBrains Mono',monospace" }}>
                ~/.hermes/sessions/ — 47 sessions, 12.3 MB
              </div>
            </div>
            <div style={{ padding: "8px 0" }}>
              <div style={{ fontSize: 12, color: SLATE.textBright, fontWeight: 500, marginBottom: 6 }}>Memory store</div>
              <div style={{ fontSize: 11, color: SLATE.muted, fontFamily: "'JetBrains Mono',monospace" }}>
                ~/.hermes/memory/ — 284 entries, 1.8 MB
              </div>
            </div>
            <div style={{
              ...
              <button
                style={{
                  padding: "6px 12px", borderRadius: 6, border: `1px solid ${SLATE.border}`,
                  background: SLATE.elevated, color: SLATE.text, fontSize: 11, cursor: "pointer",
                  fontFamily: "'JetBrains Mono',monospace",
                }}
                onMouseEnter={(e) => e.currentTarget.style.borderColor = SLATE.muted}
                onMouseLeave={(e) => e.currentTarget.style.borderColor = SLATE.border}
              >Export sessions</button>
              <button
                style={{
                  padding: "6px 12px", borderRadius: 6, border: `1px solid ${SLATE.danger}30`,
                  background: "transparent", color: SLATE.danger, fontSize: 11, cursor: "pointer",
                  fontFamily: "'JetBrains Mono',monospace",
                }}
                onMouseEnter={(e) => e.currentTarget.style.background = `${SLATE.danger}10`}
                onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}
              >Clear all data</button>
            </div>
          </SettingsSection>

          <SettingsSection title="Connections" icon="🔌">
            <TextField
              label="API Endpoint"
              value="http://192.168.178.76:3000"
              onChange={() => {}}
            ...
            />
            <div style={{ padding: "8px 0" }}>
              <div style={{ fontSize: 12, color: SLATE.textBright, fontWeight: 500, marginBottom: 6 }}>Connected services</div>
              {[
                { name: "Home Assistant", status: "connected", color: SLATE.success },
                { name: "Discord Gateway", status: "connected", color: SLATE.success },
                { name: "Grafana", status: "timeout", color: AMBER[500] },
              ].map((svc) => (
                <div key={svc.name} style={{
                  display: "flex", alignItems: "center", justifyContent: "space-between",
                  padding: "4px 0", fontSize: 11, fontFamily: "'JetBrains Mono',monospace",
                }}>
                  <span style={{ color: SLATE.text }}>{svc.name}</span>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{
                      width: 6, height: 6, borderRadius: "50%", background: svc.color,
                      boxShadow: `0 0 6px ${svc.color}`,
                    }} />
                    <span style={{ color: SLATE.muted }}>{svc.status}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Footer */}
        <div style={{
          padding: "12px 16px", borderTop: `1px solid ${SLATE.border}`,
          fontSize: 10, color: SLATE.muted, fontFamily: "'JetBrains Mono',monospace",
          display: "flex", justifyContent: "space-between",
        }}>
          <span>hermelinChat v1.0.0</span>
          <span>hermes-agent v0.9.2</span>
        </div>
      </div>
    </>
  );
...
export default function HermelinChat() {
  ...
  return (
    <div style={{
      ...
      {/* ─── SIDEBAR ──────────────────────────────────────────────── */}
      <div style={{
        width: sidebarOpen ? 250 : 48, flexShrink: 0, background: SLATE.surface,
          ...
          {sidebarOpen ? (
            ...
            <div
              ...
              style={{
                ...
                justifyContent: "center", color: SLATE.muted,
              }}
              onMouseEnter={(e) => e.currentTarget.style.color = AMBER[400]}
              onMouseLeave={(e) => e.currentTarget.style.color = SLATE.muted}
            >
        ...
        {/* Chat list */}
        <div style={{ flex: 1, overflow: "auto", padding: sidebarOpen ? "4px 6px" : "4px 0" }}>
          {sidebarOpen ? (
            <>
              <div style={{
            ...
            /* Collapsed: show tiny icons for recent sessions */
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, paddingTop: 8 }}>
              {[true, true, false, true, false].map((isAgent, i) => (
                <div key={i} style={{
                  width: 32, height: 32, borderRadius: 6, display: "flex",
                  alignItems: "center", justifyContent: "center", cursor: "pointer",
                  background: i === 0 ? `${AMBER[900]}40` : "transparent",
                  border: i === 0 ? `1px solid ${AMBER[700]}40` : "1px solid transparent",
                }}
                  onMouseEnter={(e) => { if (i !== 0) e.currentTarget.style.background = SLATE.elevated; }}
                  onMouseLeave={(e) => { if (i !== 0) e.currentTarget.style.background = "transparent"; }}
                >
                  {isAgent
                    ? <AgentIcon color={i === 0 ? AMBER[400] : SLATE.muted} />
                    : <ChatIcon color={SLATE.muted} />
                  }
                </div>
              ))}
            </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 11, fontWeight: 700, color: SLATE.bg, flexShrink: 0,
          }}>WA</div>
          {sidebarOpen && (
            <div>
